            ...
            Spec = new V1PodSpec
            {
                Containers = new List<V1Container>
                {
                    new()
                    {
                        ...
                        {
                            "sh", "-c",
                            $"echo '{script}' > script.py && echo '{inputFileBinary}' > input-file{inputType} && python -c script.py input-file{inputType} > output-file{outputType} && base64 output-file{outputType}"
                        }
                    }
                },
                RestartPolicy = "Never"
