...
public class ScriptVersionRepository(MySqlDbContext context) : IScriptVersionRepository
{
    ...
    {
        if (scriptVersion != null) await context.ScriptVersions.AddAsync(scriptVersion);
    ...
    {
        if (scriptVersion != null) context.ScriptVersions.Update(scriptVersion);
        await context.SaveChangesAsync();
    }
    ...
    {
        var scriptVersion = await context.ScriptVersions.FirstOrDefaultAsync(x => x.ScriptVersionId == id);
