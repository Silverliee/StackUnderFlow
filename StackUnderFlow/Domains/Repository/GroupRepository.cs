...
public class GroupRepository(MySqlDbContext context) : IGroupRepository
{
    ...
    {
        var groupRequests = await context.GroupRequests.Where(g => g.GroupId == groupId && g.Status == "Accepted")
            .Include(gr => gr.User)
            .ToListAsync();
    ...
    {
        return await context.GroupRequests.FirstOrDefaultAsync(gr => gr.UserId == userId && gr.GroupId == groupId);
    }
